...
export class LoginPanel {
    create(gameUri) {
        ...
        let panel = $('<dialog/>', { style: 'display:flex; flex-direction: column; width:400px; overflow:hidden' })
            ...
            .append($('<input/>'))
            .append($('<button/>', { style: '' }).text('Log In').click((event) => getJelem(event.target, LoginPanel).wrap().login()))
            .on('close', (event) => getJelem(event.target, LoginPanel).remove()) // remove itself when closed
